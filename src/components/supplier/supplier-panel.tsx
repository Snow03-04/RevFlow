...
export function SupplierPanel({ data }: { data: SupplierData | null }) {
  ...
  function apply() {
    ...
    setApplied(null);
    startApply(async () => {
      const r = await applySupplierCosts(storeId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2 rounded-xl border border-border bg-card p-5">
        <label htmlFor="supplier-store" className="text-sm font-medium">
          Loja deste separador
        </label>
        <select
          id="supplier-store"
          value={storeId}
        ...
        <p className="text-xs text-muted-foreground">
          O custo exato da sheet tem prioridade por encomenda, paga ou por
      ...
      {hasCosts && data && (
        <>
          ...
          <SheetDiff
            key={`${storeId}:${comparisonVersion}:${data.url}`}
            currency={currency}
            storeId={storeId}
          />
          ...
          <div className="space-y-3 rounded-xl border border-border bg-card p-5">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm font-medium">
                  Atualizar COGS a partir da sheet
                </p>
                <p className="text-xs text-muted-foreground">
              ...
              <button
                onClick={apply}
                disabled={applying || !storeId || !!data.error}
            ...
            {applied?.ok && (
              <div className="space-y-2 rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-3">
                ...
                {!!applied.unknownOrders && (
                  <p className="text-xs text-amber-500">
                    {applied.unknownOrders} encomendas da sheet não foram
                    encontradas na loja. Confirma o separador e a sincronização
                    das encomendas.
                  </p>
                )}
          ...
          {data.unpaidOrders.length > 0 && (
            <div className="rounded-xl border border-border bg-card p-5">
              <p className="mb-3 flex items-center gap-2 text-sm font-medium">
                <Wallet className="h-4 w-4 text-rose-500" /> Encomendas por
                pagar
