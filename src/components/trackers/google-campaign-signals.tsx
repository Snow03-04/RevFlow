import { Flame, TrendingUp, Search, History } from "lucide-react";
...
export function ScaleBadge({ level }: { level: ScaleLevel }) {
  ...
  const Icon = level === "ready" ? Flame : level === "scale" ? TrendingUp : Search;
  return <span className={cn("inline-flex items-center gap-1 whitespace-nowrap rounded-full border px-2 py-0.5 text-[10px] font-semibold",
    level === "ready" ? "scale-fire border-orange-400/60 bg-orange-500/15 text-orange-300" : level === "scale" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-400" : "border-sky-500/30 bg-sky-500/10 text-sky-400")}>
    <Icon aria-hidden className="h-3 w-3" />{level === "ready" ? "Scale pronto" : level === "scale" ? "Scale" : "Analisar scale"}
  </span>;
}
