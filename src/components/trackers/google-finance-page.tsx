...
import { GooglePerformance, PartialValue, googleStatusLabel, googleAdMoney, GoogleSpendNotice } from "./google-performance";
...
export async function GoogleFinancePage({ db, userId, sp, year, month, currency, feesByMonth, range, query }: {
  ...
  const roasFor = (campaigns: { key: string }[]) => summariseGoogleSignalRoas(campaigns.map((c) => insights.signals.get(c.key)));
  ...
  return <div className="space-y-5">
    ...
    <p className="text-xs text-muted-foreground">Nesta área, os gastos e o lucro usam o custo bruto dos anúncios, antes do crédito Google. O crédito é descontado apenas na Dashboard e na P&L principal.</p>
    <p className="text-xs text-muted-foreground">Os ROAS de campanha Google e Shopify acumulam os dias completos desde a última alteração de cada campanha. Uma nova alteração reinicia a contagem. O ROAS Google dos resumos combina o valor de conversão e o gasto desses intervalos; as folhas diárias e mensais mostram o histórico do período selecionado.</p>
    ...
    <GoogleSpendNotice coverage={campaign ? summariseGooglePnl(data.days.filter((d) => d.key === campaign.key), (d) => feesByMonth[Number(d.slice(5, 7)) - 1]).adCoverage : selected ? summariseCollection(selected.days).adCoverage : total.adCoverage} />
    {campaign ? <>
      <GoogleCampaignSignals signal={insights.signals.get(campaign.key)} changes={insights.changes.filter((c) => c.campaign_key === campaign.key)} today={insights.today} currency={currency} expanded />
      ...
      <GooglePnlSheet campaign={campaign} rows={data.days.filter((d) => d.key === campaign.key)} year={year} month={month} currency={currency} feesByMonth={feesByMonth} query={query} googleRoasSinceChange={insights.signals.get(campaign.key)?.google.roas} />
    </> : selected ? <>
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">{[
        ...
        [total.complete ? "Lucro das coleções" : "Lucro identificado · parcial", money(total.spendKnown && collections.some((c) => c.days.length) ? total.profit : null, currency), total.profit == null ? "" : total.profit < 0 ? "text-red-400" : "text-emerald-400"],
      ...
      <GooglePerformance metrics={total} currency={currency} googleRoasSinceChange={roasFor(campaigns)} />
      ...
      {!stores.length ? <Card className="p-6 text-sm text-muted-foreground">Ainda sem atividade Google neste período. Importa os dados com o script disponível em Connections.</Card> : stores.map(([id, name]) => {
        ...
        return <Card key={id ?? "unmapped"} className="overflow-hidden"><details open className="group/store">
          <summary className="flex cursor-pointer list-none flex-wrap items-center justify-between gap-3 border-b border-border bg-muted/20 px-4 py-3"><span className="flex items-center gap-2 text-sm font-medium"><ChevronRight className="h-3.5 w-3.5 transition-transform group-open/store:rotate-90" />{name}</span><span className="flex flex-wrap gap-x-5 gap-y-1 text-xs text-muted-foreground"><span>Gasto bruto <strong className="ml-1 text-foreground">{googleAdMoney(s.adCoverage, "grossSpend", currency)}</strong></span><span>{s.impressions.toLocaleString("pt-PT")} impressões</span><span>{s.clicks.toLocaleString("pt-PT")} cliques</span><span>CTR {pct(s.impressions ? s.clicks / s.impressions : null)}</span><span>CPC {googleAdMoney(s.adCoverage, "cpc", currency)}</span></span></summary>
          <div className="overflow-x-auto scrollbar-thin"><div className="min-w-[760px]">
            <div className="grid grid-cols-[minmax(200px,2.3fr)_55px_repeat(4,minmax(85px,1fr))_75px] gap-3 border-b border-border px-4 py-2 text-right text-[10px] uppercase tracking-wide text-muted-foreground"><span className="text-left">Coleção</span><span>Camp.</span><span>Gasto bruto</span><span>Faturação</span><span>Lucro</span><span>ROAS / equil.</span><span /></div>
            {groups.map((c) => { const r = summariseCollection(c.days); return <div key={c.key} className="relative border-b border-border last:border-0"><details className="group/collection">
              <summary className="grid cursor-pointer list-none grid-cols-[minmax(200px,2.3fr)_55px_repeat(4,minmax(85px,1fr))_75px] items-center gap-3 px-4 py-3 text-right text-xs hover:bg-muted/20"><span className="flex min-w-0 items-center gap-2 text-left font-medium"><ChevronRight className="h-3 w-3 shrink-0 text-muted-foreground transition-transform group-open/collection:rotate-90" /><span className="truncate">{c.name}</span></span><span className="text-muted-foreground">{c.campaigns.length}</span><span className="tabular-nums">{googleAdMoney(r.adCoverage, "grossSpend", currency)}</span><span className="tabular-nums">{money(r.revenue, currency)}</span><span className={cn("tabular-nums", r.profit != null && (r.profit < 0 ? "text-red-400" : "text-emerald-400"))}><PartialValue partial={!r.complete && r.spendKnown && c.days.length > 0}>{money(r.spendKnown && c.days.length ? r.profit : null, currency)}</PartialValue></span><span className="whitespace-nowrap tabular-nums">{mult(r.roas)}<span className="text-[10px] text-muted-foreground"> / {mult(r.breakEven)}</span></span><span /></summary>
              <div className="space-y-4 border-t border-border bg-muted/10 px-4 py-4"><GooglePerformance metrics={r} currency={currency} googleRoasSinceChange={roasFor(c.campaigns)} /><div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-muted-foreground"><span>Gasto bruto {googleAdMoney(r.adCoverage, "grossSpend", currency)}</span><span>{r.orders} encomendas Shopify</span><span>COGS {money(r.cogs, currency)} · {pct(r.cogsPct)}</span><span>Margem identificada {pct(r.spendKnown && c.days.length ? r.margin : null)}{!r.complete ? " · parcial" : ""}</span></div>{r.reasons.length > 0 && <p className="text-xs text-amber-400">{r.reasons.join(". ")}. O lucro parcial usa apenas as vendas identificadas.</p>}{!c.days.length && <p className="text-xs text-muted-foreground">Sem atividade importada neste período.</p>}{campaignList(c)}</div>
