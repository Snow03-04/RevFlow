...
export function GeneralCollectionSheet({ collection, year, month, currency, feesByMonth, query }: {
  ...
  return <section className="space-y-4">
    ...
    <p className="text-xs leading-relaxed text-muted-foreground">Conta a parte paga da encomenda quando contém pelo menos um produto pago desta coleção, qualquer que seja a origem. Inclui os extras pagos do AfterSell, portes, reembolsos, custos e taxas. Se um extra ficar por pagar, a compra original continua a contar quando o pagamento e os artigos estão confirmados. Encomendas pendentes ou apenas autorizadas ficam de fora. Cada encomenda conta uma vez por coleção; as sheets de coleções diferentes podem partilhar encomendas. Lucro = vendas líquidas − COGS − anúncios Meta e Google − taxas e comissões. O custo Google é bruto, antes de créditos.</p>
    ...
    ].map((label, index) => <TableHead key={label} data-detail={[2, 4, 6, 7, 9, 10].includes(index)} className={cn("whitespace-nowrap text-[11px] uppercase", index > 0 && "text-right")}>{label}</TableHead>)}</TableRow></TableHeader><TableBody>
  ...
  function cells(summary: ReturnType<typeof summariseGeneralSheet>, active: boolean) {
    ...
    return values.map((value, index) => <TableCell key={index} data-detail={[1, 3, 5, 6, 8, 9].includes(index)} className={cn("whitespace-nowrap text-right text-xs tabular-nums", index === 11 && active && summary.profit != null && (summary.profit < 0 ? "text-red-400" : "text-emerald-400"))}>{active ? value : "—"}</TableCell>);
  }
}
