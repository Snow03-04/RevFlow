...
export function GooglePnlSheet({ campaign, rows, year, month, currency, feesByMonth, query }: {
  ...
  function cells(s: ReturnType<typeof summariseGooglePnl>, active: boolean) {
    return [money(s.grossSpend, currency), s.impressions.toLocaleString("pt-PT"), s.clicks.toLocaleString("pt-PT"), pct(s.ctr), money(s.cpc, currency), s.conversions.toLocaleString("pt-PT"), money(s.conversionValue, currency), mult(s.googleRoas),
