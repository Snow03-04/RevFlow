...
export async function GeneralFinancePage({ db, userId, sp, year, month, currency, feesByMonth, range, query }: {
  ...
  return <div className="space-y-5">
    ...
    <p className="max-w-4xl text-xs leading-relaxed text-muted-foreground">Uma folha por coleção e por loja. Junta os anúncios Meta e Google e conta o valor completo das encomendas pagas que contêm produtos da coleção, incluindo AfterSell e vendas de qualquer canal.</p>
    ...
    <details className="text-xs text-muted-foreground"><summary className="cursor-pointer">Como funciona a General sheet</summary><div className="mt-2 max-w-4xl space-y-2 leading-relaxed"><p>As coleções são identificadas pelos destinos dos anúncios ou pela associação que definires. Os produtos são confirmados na coleção Shopify atual. Campanhas da mesma coleção, em Meta ou Google, partilham uma só folha nessa loja.</p><p>Cada encomenda conta uma vez por coleção, com o valor completo: todos os artigos, extras do AfterSell e portes, descontando os reembolsos, custos e taxas da encomenda inteira. Uma encomenda com produtos de duas coleções aparece nas duas folhas, pelo que as folhas não devem ser somadas como total da loja.</p><p>Os gastos incluem o histórico das campanhas associadas à coleção, incluindo campanhas entretanto pausadas. O lucro desconta COGS, gasto bruto dos anúncios, taxas de pagamento e comissões configuradas na P&amp;L.</p></div></details>
  </div>;
}
