...
test("Dashboard keeps Elena selected across preset and custom date changes", () => {
  const initial = "store=elena&period=custom&from=2026-07-01&to=2026-07-31";
  for (const period of ["today", "last7", "month", "year"]) {
    const params = new URL(
      dashboardPeriodUrl(initial, period),
      "http://localhost",
    ).searchParams;
    assert.equal(params.get("store"), "elena");
